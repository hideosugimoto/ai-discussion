...
export function formatPlanForModel(plan, modelId, nameOf) {
  ...
  // 選ぶ。実測（2026-09-25）では、初回を「台帳が空のとき」と条件付けたところ
  // ラウンド2で再発した（2体が同一施設を選んだ）。対象を新しく選ぶ場面は毎
  // ラウンド訪れるので、条件を外してAIごとに決定論的な順位を割り当てる。
  if (ordinal > 0 && ids.length > 1) {
    parts.push(
      `\n【対象の選び方】あなたは${ids.length}者のうち${ordinal}番目の担当です。`
      + `台帳に無い新しい対象を選ぶときは毎回、検索結果の上位1件に飛びつかないでください。`
      + `条件に合う候補を${ids.length}件以上挙げ、そこから台帳に既にあるものを除き、`
      + `残った候補の${ordinal}番目を選んでください。`
      + `他AIも同じラウンドで同じ検索をしています。対象が重複した分の調査は捨てられます。`,
    );
  }
  return parts.join("\n");
}
