...
import { callClaude, callChatGPT, callGemini, validateClaude, validateChatGPT, validateGemini } from "./api";
import { encryptSettings, decryptSettings } from "./crypto";
import { loadSettings, saveSettings } from "./storage";

// ── Sub-components ─────────────────────────────────────────────

function ModelBadge({ model, tag, size = "md" }) {
  const sm = size === "sm";
  return (
    <div style={{ display:"inline-flex", alignItems:"center", gap:5, padding:sm?"3px 8px":"5px 12px", borderRadius:20, background:`${model.color}18`, border:`1px solid ${model.color}40` }}>
      ...
      {tag && <span style={{ color:`${model.color}80`, fontSize:10 }}>{tag}</span>}
    </div>
  );
}
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:6, animation:isNew&&!msg.loading?"fadeIn 0.4s ease":"none" }}>
      ...
      <div style={{ background:model.bg, border:`1px solid ${model.dimColor}`, borderLeft:`3px solid ${model.color}`, borderRadius:"0 10px 10px 10px", padding:"12px 16px", color:"#e2e8f0", fontSize:13.5, lineHeight:1.8, whiteSpace:"pre-wrap", minHeight:48 }}>
        {msg.error
          ...
          : <span style={{ color:"#ffffff30", animation:"pulse 1.2s infinite" }}>考えています...</span>
        }
      </div>
    </div>
  );
...
function RoundSection({ round, roundNum, isLatest }) {
  return (
...
function Collapsible({ label, badge, hint, open, onToggle, children }) {
  return (
    <div style={{ marginBottom:10 }}>
      <button onClick={onToggle} style={{ background:"none", border:"1px solid #2a2a3a", borderRadius:8, padding:"7px 14px", color:badge?"#4ade8090":"#ffffff50", cursor:"pointer", fontSize:12, fontFamily:"monospace", display:"flex", alignItems:"center", gap:8 }}>
        <span>{open?"▾":"▸"}</span>
        <span>{label}</span>
...
export default function App() {
  ...
  const [saveKeys, setSaveKeys] = useState(saved.saveKeys ?? false); // デフォルトOFF
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (saveKeys) saveSettings({ keys:result.keys||keys, saveKeys, profile:result.profile||profile });
      setCryptoMsg("✓ 復元完了！");
      setImportText("");
      setImportPw("");
    ...
    <div style={{ minHeight:"100vh", background:"#09090f", color:"#e2e8f0", display:"flex", flexDirection:"column", alignItems:"center", padding:"24px 16px 80px" }}>
      ...
      {profileNotice && (
        <div style={{ width:"100%", maxWidth:720, marginBottom:12, padding:"10px 16px", background:"#1c1a07", border:"1px solid #78580f", borderRadius:8, display:"flex", justifyContent:"space-between", alignItems:"center", gap:12 }}>
          ...
          <button onClick={() => { setProfileNotice(false); sessionStorage.setItem("profile-notice-dismissed","1"); }} style={{ background:"none", border:"none", color:"#f59e0b", cursor:"pointer", fontSize:16, padding:"0 4px", flexShrink:0 }}>✕</button>
        </div>
      )}
      ...
      <div style={{ width:"100%", maxWidth:720 }}>
        ...
        <div style={{ display:"flex", gap:8, marginBottom:10 }}>
          <div style={{ display:"flex", background:"#10101a", border:"1px solid #2a2a3a", borderRadius:8, overflow:"hidden" }}>
            {[{id:"best",label:"🧠 最強"},{id:"fast",label:"⚡ 高速"}].map(({id,label}) => (
              <button key={id} onClick={() => setMode(id)} style={{ padding:"6px 14px", border:"none", cursor:"pointer", fontSize:12, fontWeight:600, background:mode===id?"#7c3aed":"transparent", color:mode===id?"#fff":"#ffffff50" }}>{label}</button>
            ))}
          </div>
        </div>
        ...
        <Collapsible label="APIキー" badge={allKeysSet?"✓ 全て設定済":null} hint={!allKeysSet?"⚠ 未設定あり":null} open={showKeys} onToggle={() => setShowKeysPanel((s)=>!s)}>
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            {keyConfigs.map(({id,label,ph,link}) => (
              <div key={id}>
                ...
                <div style={{ display:"flex", gap:6 }}>
                  <input type="password" value={keys[id]} onChange={(e) => updateKey(id, e.target.value)} placeholder={ph}
                  ...
                  <button onClick={() => validateKey(id, keys[id])} disabled={!keys[id]}
            ...
            <div style={{ padding:"10px 12px", background:"#0d0d1a", border:"1px solid #2a2a3a", borderRadius:8, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              ...
              <button onClick={() => toggleSaveKeys(!saveKeys)} style={{ padding:"6px 16px", border:"none", borderRadius:20, cursor:"pointer", fontSize:12, fontWeight:700, background:saveKeys?"#16a34a":"#2a2a3a", color:saveKeys?"#fff":"#ffffff50" }}>
                {saveKeys ? "ON" : "OFF"}
              </button>
            </div>
        ...
        {/* Security */}
        <Collapsible label="🔒 セキュリティについて" open={showSecurity} onToggle={() => setShowSecurity((s)=>!s)}>
        ...
        {/* Profile */}
        <Collapsible label="あなたのプロフィール" badge={profile.trim() ? (profileUpdatedAt ? (() => { const d = Math.floor((Date.now() - new Date(profileUpdatedAt)) / (1000*60*60*24)); return d >= 30 ? `⚠️ 設定済（${d}日前に更新）` : `✓ 設定済（${d}日前に更新）`; })() : "✓ 設定済") : null} hint={!profile.trim()?"任意":null} open={showProfile} onToggle={() => setShowProfile((s)=>!s)}>
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            <div style={{ fontSize:11, color:"#ffffff40" }}>各AIのシステムプロンプトに自動注入。Claude.aiで「今まで把握している私の情報をまとめて」と聞いた内容をそのまま貼るのがおすすめ。</div>
            <textarea value={profile} onChange={(e) => updateProfile(e.target.value)}
            ...
            {profile.trim() && <button onClick={() => updateProfile("")} style={{ alignSelf:"flex-end", background:"none", border:"1px solid #3a2a2a", borderRadius:6, padding:"4px 12px", color:"#ef444460", cursor:"pointer", fontSize:11 }}>クリア</button>}
          </div>
        </Collapsible>
        ...
        <Collapsible label="🔐 設定の暗号化バックアップ" open={showSave} onToggle={() => setShowSave((s)=>!s)}>
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            ...
            <div>
              ...
              <input type="password" value={exportPw} onChange={(e) => setExportPw(e.target.value)} placeholder="バックアップ用パスワードを設定"
            ...
            <div>
              ...
              <textarea value={importText} onChange={(e) => setImportText(e.target.value)} placeholder="バックアップテキストを貼り付け" rows={3}
              ...
              <input type="password" value={importPw} onChange={(e) => setImportPw(e.target.value)} placeholder="バックアップ時に設定したパスワード"
                style={{ width:"100%", background:"#09090f", border:"1px solid #2a2a3a", borderRadius:6, padding:"8px 10px", color:"#e2e8f0", fontSize:13, fontFamily:"monospace", marginBottom:8 }} />
        ...
        {showIntervention && !running && discussion.length > 0 && (
          <div style={{ marginTop:16, display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ background:"#10101a", border:"1px solid #4c1d95", borderRadius:12, overflow:"hidden" }}>
              <textarea value={intervention} onChange={(e) => setIntervention(e.target.value)}
