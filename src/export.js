...
// 調査モードの成果物。レポート本文と、その根拠になった台帳をそのまま出す。
// エクスポートに含めないと、作った調査結果がアプリの外に持ち出せない。
function researchMarkdown(research) {
  const report = (research?.report || "").trim();
  const ledger = Array.isArray(research?.ledger) ? research.ledger : [];
  if (!report && ledger.length === 0) return [];
  const lines = [];
  if (report) {
    lines.push(`## 📄 調査レポート`);
    lines.push(sanitize(report));
    lines.push("");
  }
  if (ledger.length) {
    lines.push(`## 🔬 確定事実台帳（${ledger.length}件）`);
    lines.push(`| 対象 | 項目 | 値 | 確度 | 出典 |`);
    lines.push(`|---|---|---|---|---|`);
    ledger.forEach((e) => {
      const cell = (v) => sanitize(v).replace(/\|/g, "\\|");
      lines.push(`| ${e.conflict ? "⚠️ " : ""}${cell(e.target)} | ${cell(e.field)} | ${cell(e.value)} | ${cell(e.confidence)} | ${e.url ? cell(e.url) : "出典なし"} |`);
    });
    lines.push("");
  }
  return lines;
...
function researchHtml(research) {
  ...
  const parts = [];
  if (report) {
    parts.push(`<div style="font-size:15px;font-weight:700;color:#1e3a8a;margin-bottom:8px">📄 調査レポート</div>`);
    parts.push(`<div class="md-body" style="font-size:13px;line-height:1.8;color:#333">${renderMarkdownHtml(report)}</div>`);
  }
  if (ledger.length) {
    const rows = ledger.map((e) => `<tr>
      <td style="padding:4px 8px;border-top:1px solid #e5e7eb">${e.conflict ? "⚠️ " : ""}${sanitizeHtml(e.target)}</td>
      <td style="padding:4px 8px;border-top:1px solid #e5e7eb">${sanitizeHtml(e.field)}</td>
      <td style="padding:4px 8px;border-top:1px solid #e5e7eb">${sanitizeHtml(e.value)}</td>
      <td style="padding:4px 8px;border-top:1px solid #e5e7eb;white-space:nowrap">${sanitizeHtml(e.confidence)}</td>
      <td style="padding:4px 8px;border-top:1px solid #e5e7eb;word-break:break-all">${e.url ? `<a href="${sanitizeHtml(e.url)}" target="_blank" rel="noopener noreferrer">出典</a>` : "出典なし"}</td>
    </tr>`).join("");
    parts.push(`<div style="font-size:14px;font-weight:700;color:#1e3a8a;margin:14px 0 6px">🔬 確定事実台帳（${ledger.length}件）</div>`);
    parts.push(`<table style="width:100%;border-collapse:collapse;font-size:12px"><tbody>${rows}</tbody></table>`);
  }
  return `<section style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:10px;padding:16px;margin-bottom:20px">${parts.join("")}</section>`;
}

export function exportToMarkdown(topic, discussion, summaries, personas, verdict, actionPlan, research) {
  const lines = [];
  ...
  lines.push(...researchMarkdown(research));
...
export function exportToHtml(topic, discussion, summaries, personas, verdict, actionPlan, research) {
...
${researchHtml(research)}${verdictHtml(verdict)}${actionPlanHtml(actionPlan)}${rounds}
...
export function downloadHtml(topic, discussion, summaries, personas, verdict, actionPlan, research) {
  const html = exportToHtml(topic, discussion, summaries, personas, verdict, actionPlan, research);
  const blob = new Blob([html], { type: "text/html;charset=utf-8" });
  triggerDownload(blob, buildFileName(topic, "html"));
}

export function downloadMarkdown(topic, discussion, summaries, personas, verdict, actionPlan, research) {
  const md = exportToMarkdown(topic, discussion, summaries, personas, verdict, actionPlan, research);
