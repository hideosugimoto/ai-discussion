...
| モード | 表示 | 非表示 | 用途 |
|--------|------|--------|------|
| シンプル ○ | APIキー・プロフィール・履歴のみ、1列表示 | ペルソナ・議論モード・憲法・バックアップ・セキュリティ | 初めて使う人、シンプルに使いたい時 |
| 通常 ◉ | 全機能表示、3列レイアウト | なし | 普段使い（デフォルト） |
| 詳細 ◎ | 全機能+ワイドレイアウト、コンパクト間隔、下部固定コントロール | なし | ヘビーユーザー、情報を最大限表示 |

テーマ（配色）とUIモード（情報量・レイアウト）は独立して切り替え可能です。
